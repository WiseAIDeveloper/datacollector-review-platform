...
def read_reviews(path):
    """Return persisted quality decisions, or an empty mapping before the first review."""
    path = Path(path)
    return json.loads(path.read_text()) if path.exists() else {}


def save_review(path, row, status, notes, expected, root):
    """Save a checked quality decision and restore indexes if review persistence fails."""
    if status not in {"pass", "error", "unreviewed"}:
        raise ValueError("Invalid quality status")
    if not isinstance(notes, str) or len(notes) > 4096:
        raise ValueError("Notes must be at most 4096 characters")
    reviews = read_reviews(path)
    key = row["key"]
    if reviews.get(key) != expected:
        raise Conflict("Quality review changed; refresh before saving")
    entry = dict(
        key=key,
        folder=row["folder"],
    ...
    originals = [
        (
            ...
            (folder / name).stat().st_mode & 0o777,
        )
        for name in INDEXES
    ]
    ...
    edit_capture(
        ...
        {"quality_review_status": value},
        row["metadata"],
        allowed_fields={"quality_review_status"},
    )
    try:
        atomic_write(
            path,
        ...
        for index, content, mode in originals:
            atomic_write(index, content, mode)
        raise
    return entry
