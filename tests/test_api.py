...
class ApiTests(unittest.TestCase):
    ...
    def test_read_routes_and_headers(self):
        ...
        for filename in ["index.html", "coverage.html", "quality.html", "search.html",
                         "ingestion.html", "capture_review.js", "image_zoom.js",
                         "terminal.css", "frozen_panes.css"]:
        ...
        self.assertEqual(rows[0], dict(key=self.key, folder="genuine", line=2, sdk="web",
                                      device="iphone-13", annotation_lighting="office-white", metadata=self.row))
        self.assertEqual(self.client.request("/api/capture?key=" + quote(self.key))[1], rows[0])
        self.assertEqual(self.client.request("/api/image?key=" + quote(self.key))[1], IMAGE)
        self.assertEqual(self.client.request("/api/annotation?key=" + quote(self.key))[1],
                         {"lighting": "office-white", "uuid": self.row["uuid"]})
        self.assertEqual(self.client.request("/api/matrix")[1][0]["matrix_name"], MATRIX_NAME)
        self.assertEqual(self.client.request("/api/batches")[1][0]["batch_name"], "genuine")
    ...
    def test_search_and_not_found(self):
        ...
        self.assertEqual(self.client.request("/api/search?q=missing")[1], {"total": 0, "results": []})
        for query in ("", "x" * 201):
            self.assertEqual(self.client.request("/api/search?q=" + query)[0], 400)
        for path in ("/missing", "/api/capture?key=missing", "/api/image?key=missing", "/.env", "/server.py"):
    ...
    def test_authentication_and_payload_limits(self):
        ...
        self.assertEqual(self.client.request("/api/quality", raw=b"")[0], 400)
        self.assertEqual(self.client.request("/api/quality", {}, headers={"Content-Length": "1048577"})[0], 400)
    ...
    def test_ingestion_and_pagination(self):
        ...
        self.assertEqual(status, 200)
        self.assertEqual((snapshot["total"], snapshot["existing"], snapshot["ingested"]), (5, 5, 0))
        self.assertTrue(all(event["available"] for event in snapshot["events"]))
        next_page = self.client.request(f"/api/ingestion?limit=2&before={snapshot['next_before']}")[1]
        self.assertFalse({e["id"] for e in snapshot["events"]} & {e["id"] for e in next_page["events"]})
        self.assertEqual(len(self.client.request("/api/ingestion?limit=0")[1]["events"]), 1)
    ...
    def test_edit_conflict_and_action_history(self):
        ...
        payload = dict(folder="genuine", uuid=self.row["uuid"], filename=self.row["filename"],
                       changes={"lighting": "office-yellow"}, expected=self.row)
        self.assertEqual(self.client.request("/api/edit-capture", payload)[1],
        ...
        self.assertEqual(snapshot["actions"][0]["changes"], {"lighting": {"from": "dark", "to": "office-yellow"}})
        self.assertEqual(len((self.client.root / "actions.jsonl").read_text().splitlines()), 1)
    ...
    def test_delete_validation_and_preserved_history(self):
        ...
        item = dict(key=self.key, folder="genuine", uuid=self.row["uuid"], filename=self.row["filename"])
        for payload in ({}, {"remove": "invalid"}, {"remove": [item], "confirm_count": 0},
                        {"remove": [item, item], "confirm_count": 2}):
            self.assertEqual(self.client.request("/api/apply-decisions", payload)[0], 400)
        self.assertEqual(self.client.request("/api/apply-decisions", {"remove": [{**item, "key": "stale"}], "confirm_count": 1})[0], 409)
        status, result, _ = self.client.request("/api/apply-decisions", {"remove": [item], "confirm_count": 1})
        self.assertEqual((status, result["deleted"]), (200, 1))
        self.assertTrue(result["results"][0]["json_preserved"])
        self.assertEqual(self.client.request("/api/capture?key=" + quote(self.key))[0], 404)
        self.assertTrue((self.client.root / "genuine/mykadfront/datacollector_annotation" / f"{self.row['uuid']}.json").exists())
        snapshot = self.client.request("/api/ingestion")[1]
        self.assertEqual(snapshot["total"], 5)
        self.assertFalse(next(e for e in snapshot["events"] if e["key"] == self.key)["available"])
    ...
    def test_quality_validation_conflicts_and_persistence(self):
        ...
        payload = dict(key=self.key, status="error", notes="Synthetic glare", expected=None)
        self.assertEqual(self.client.request("/api/quality", {**payload, "status": "invalid"})[0], 400)
        self.assertEqual(self.client.request("/api/quality", {**payload, "key": "missing"})[0], 409)
        expected = None
        for status, value in (("error", "fail"), ("pass", "pass"), ("unreviewed", "")):
            response, entry, _ = self.client.request("/api/quality", {**payload, "status": status, "expected": expected})
            ...
            for name in INDEXES:
                ...
                with (self.client.root / "genuine" / name).open() as stream:
                    self.assertEqual(next(csv.DictReader(stream))["quality_review_status"], value)
            expected = entry
