    ...
    def test_application_instances_do_not_share_dataset_or_secret(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertEqual(
                    [len(s.application.records()) for s in servers], [1, 2]
                )
                self.assertIsNot(
                    servers[0].application.lock, servers[1].application.lock
                )
                self.assertNotEqual(
                    servers[0].application.settings.delete_token,
                    servers[1].application.settings.delete_token,
                )
                self.assertTrue(
                    all(s.application.ingestion.thread is None for s in servers)
                )
            finally:
