...
with sync_playwright() as p:
    ...
    def route(request):
        ...
        state["calls"] += 1
        if state["fail"]:
            request.fulfill(status=503, body="test outage")
            return
        older = "before=" in request.request.url
        name = (
            "older.jpg" if older else "new.jpg" if state["calls"] > 1 else "first.jpg"
        )
        event = dict(
            id=1,
            status="ingested",
            ...
            sdk="web",
            device="iphone-13",
            test_plan="fixture_plan",
            creation_time="capture time",
        )
        request.fulfill(
            json=dict(
                last_scan=event["detected_at"],
                errors=[],
                pending_images=0,
                total=1,
                existing=0,
                ingested=1,
                events=[event],
                next_before=None if older else 1,
            )
        )
    ...
    expect(page.locator("#logs")).to_contain_text("older.jpg")
    page.locator("#latest").click()
    expect(page.locator("#logs")).to_contain_text("new.jpg")
    state["fail"] = True
    page.locator("#refresh").click()
    expect(page.locator("#error")).to_contain_text("503")
    state["fail"] = False
    page.locator("#refresh").click()
    ...
    for path in ["/", "/coverage.html"]:
        page.goto(BASE + path)
        expect(
            page.locator(".app-sidebar").get_by_role("link", name="Ingestion logs")
        ).to_have_attribute("href", "/ingestion.html")
    browser.close()
print(
    "PASS: 5-second refresh, compact dark style, hidden event status/time, pagination, outage recovery, sidebar links."
)
