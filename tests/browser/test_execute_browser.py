...
with sync_playwright() as p:
    ...
    page.route(
        "**/review",
        lambda r: r.fulfill(
            body=asset_path(src, "index.html").read_text(), content_type="text/html"
        ),
    )
    fail = [False]
    ...
    page.goto("http://fixture/review")
    page.locator("article").first.wait_for()
    expect(page.locator("article")).to_have_count(5)
    page.locator("article").first.get_by_role("button", name="Keep", exact=True).click()
