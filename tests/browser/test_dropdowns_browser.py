...
row = next(
    ...
    if any(
        ...
        and b["test_plan_name"] == r["metadata"]["test_plan_name"]
        for b in batches
    )
)
batch = next(
    b
    for b in batches
    if b["batch_name"] == row["folder"]
    and b["test_plan_name"] == row["metadata"]["test_plan_name"]
)
with sync_playwright() as p:
    ...
    def values(field):
        """Read the available metadata choices from the fixture dropdown."""
        return page.locator("#edit-" + field + " option").evaluate_all(
    ...
    browser.close()
print(
    "PASS dropdown types, batch choices, App blank device, configured identities, changed CSV options, preserved current metadata. No writes."
)
